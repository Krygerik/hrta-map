...
--		end
    ...
    if init_mana ~=  GetUnitManaPoints(unit) and ( IsNamedHero(ally_hero, 'Quroq') or IsNamedHero(ally_hero, 'Quroq2') ) then
      ...
      for i, creature in GetCreatures(GetUnitSide(unit)) do
        local type = GetCreatureType(creature)
        --проверка на виверн
        if type ~= 127 and  type ~= 128 and type ~= 178 then
      ...
      print("randomCreatureNum")
      print(randomCreatureNum)
      print("creaturesWhoCanGetCallOfBlood")
      print(creaturesWhoCanGetCallOfBlood)
      if creaturesWhoCanGetCallOfBlood[randomCreatureNum] ~= nil then
        ...
        UnitCastAimedSpell(unit, SPELL_WARCRY_CALL_OF_BLOOD, creaturesWhoCanGetCallOfBlood[randomCreatureNum])
