  ...
  for _, armyId in stash do
    for wizardId, wizardLvl in dictUnitsWizard do
      if armyId == wizardId then
        ...
        AddHeroCreatures(mainHeroName, wizardId, intg(unitInTown.count * DIPLOMACY_COEF));
...
function vaishanSpec(heroName)
  ...
  if GetHeroCreatures(heroName, 92) >= GetHeroCreatures(heroName, 93) then
    AddHeroCreatures(heroName, 92, floor(heroLevel * VAISHAN_BONUS_BY_LEVEL));
  else
    AddHeroCreatures(heroName, 93, floor(heroLevel * VAISHAN_BONUS_BY_LEVEL));
  end;
end;
